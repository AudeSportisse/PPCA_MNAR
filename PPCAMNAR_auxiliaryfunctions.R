...
Results_graph <- function(Covar,Moy,YNA,Y,B,indcolNA,M,r,p,n,sigma){ 
  ...
  for (j in 1:length(indcolNA)){
    ...
    remp2 <- ginv(Cov[enscol, enscol],tol = 10^-20) #solve(Cov[enscol, enscol]) 
...
Mean_graphical_algebraic <- function(indcolNA,indcolReg,j,YNA,YNAcc,r){
  ...
  for (u0 in 1:length(indcolReg)){
    ...
    if(r-1-length(indcolReg_mselec1)==0){
      ...
      sumMean <- c()
      for (ind in 1:length(indcolReg_mselec1)){ sumMean <- c(sumMean,coefficients(regcc)[ind+1] * mean(YNA[,indcolReg_mselec1[ind]]))}
...
Variance_algebraic <- function(Mean,meth,indcolNA,indcolReg,j,YNA,YNAcc,CovTheo,r,sigma){
...
Covariance_obsmiss_algebraic <- function(Mean,meth,indselec,indcolNA,indcolReg,j,YNA,YNAcc,CovTheo,r,sigma){
...
Covariance_2missing_algebraic <- function(Variance,CovarianceMat,meth,indcolNA,indcolReg,j,jother,YNA,YNAcc,CovTheo,r,sigma){
...
PPCA_MAREM_it <- function(B_estimOld,B,moy_estim,YNA,sigma,r){
