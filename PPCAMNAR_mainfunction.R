...
ComparMethods_PPCA_iteration <-
  function(seed_num,
    ...
    if (simplify == TRUE){
      indcolReg <- sample(indcolObs,r_reg,replace=FALSE)
    }else{
      indcolReg <- indcolObs
    }
    ...
    for (j in 1:length(indcolNA)){
      ...
      if (agg==1){
        resVarAlg <- Variance_algebraic(MeanMNAR_agg,"agg",indcolNA,indcolReg,j,YNA,YNAcc,CovTheo,r,sigma)
        VarMNAR_agg[j] <- resVarAlg[1]
        CovMNAR_agg[indcolNA[j],indcolNA[j]] <-  VarMNAR_agg[j]
      }

      if (noagg==1){
        resVarAlg <- Variance_algebraic(MeanMNAR,"noagg",indcolNA,indcolReg,j,YNA,YNAcc,CovTheo,r,sigma)
      ...
      while(length(indcol_cov)>0){
        ...
        if (agg==1){
          resCovAlg <- Covariance_obsmiss_algebraic(MeanMNAR_agg,"agg",indselec,indcolNA,indcolReg,j,YNA,YNAcc,CovTheo,r,sigma)
          CovMNAR_agg[indselec,indcolNA[j]] <- resCovAlg[1]
          CovMNAR_agg[indcolNA[j],indselec] <-  CovMNAR_agg[indselec,indcolNA[j]]
        }

        if (noagg==1){
          resCovAlg <- Covariance_obsmiss_algebraic(MeanMNAR,"noagg",indselec,indcolNA,indcolReg,j,YNA,YNAcc,CovTheo,r,sigma)
      ...
      if(agg==1){
        for (jother in 1:(length(indcolNA)-1)){
          if(jother < j){
            resCovMissAlg <- Covariance_2missing_algebraic(VarMNAR_agg,CovMNAR_agg,"agg",indcolNA,indcolReg,j,jother,YNA,YNAcc,CovTheo,r,sigma)
      ...
      if(noagg==1){
        for (jother in 1:(length(indcolNA)-1)){
          if(jother < j){
            resCovMissAlg <- Covariance_2missing_algebraic(VarMNAR,CovMNAR,"noagg",shrink,indcolNA,indcolReg,j,jother,YNA,YNAcc,CovTheo,r,sigma)
    ...
    if(agg==1){
      res_agg <- Results_graph(CovMNARg_agg,MeanMNAR_agg,YNA,Y,B,indcolNA,M,r,p,n,sigma)
      res2_agg <- Results_graph(CovMNAR_agg,MeanMNAR_agg,YNA,Y,B,indcolNA,M,r,p,n,sigma)
    ...
    if(noagg==1){
      res_noagg <- Results_graph(CovMNARg,MeanMNAR,YNA,Y,B,indcolNA,M,r,p,n,sigma)
      res2_noagg <- Results_graph(CovMNAR,MeanMNAR,YNA,Y,B,indcolNA,M,r,p,n,sigma)
    ...
    resMAR <- Results_graph(CovMAR,MeanMAR,YNA,Y,B,indcolNA,M,r,p,n,sigma)
    ...
    B_estimNew <-  matrix(rnorm(p*r,sd=3,mean=1), nrow = r, ncol = p)
    moy_estim <- rnorm(p,sd=1,mean=3)
    seuilEM <- 10^-3
    diff <- 100
    ccompt <- 0
    
    # Algo EM
    while (diff > seuilEM | ccompt > 100){
      resPPCA_MAREM_it <- PPCA_MAREM_it(B_estimNew,B,moy_estim,YNA,sigma,r)
