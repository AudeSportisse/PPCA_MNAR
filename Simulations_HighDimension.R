...
result_hg_r2 <- lapply(1:Nbit,ComparMethods_PPCA_iteration,
                  n=n,
                  p=p,
                  r=r,
                  B=B,
...
result_hg_r5 <- lapply(1:Nbit,ComparMethods_PPCA_iteration,
                    n=n,
                    p=p,
                    r=r,
                    B=B,
...
result <- result_hg_r5 # or result_hg_r2
set.seed(2002)
Nbit <- 20
p <- 50 
r <- 5 #r <- 2
...
j <- 6 #the missing variable for which the mean, variance and covariances is ploted.
jmiss <- 4 #the missing variable for which the covariance is ploted in plot3.
jobs <- 21 #the observed variable for which the covariance is ploted in plot4.
Meanglob <- c()
Varglob <- c()
Covglob1 <- c()
Covglob2 <- c()
